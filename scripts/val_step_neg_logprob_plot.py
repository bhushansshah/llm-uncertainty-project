...
def val_step_mean_var_count_from_step_lists(
    ...
    """
    Per step index j (chunk index):
      - mean/var over correct examples that have step j (population variance, ddof=0)
      - mean/var over incorrect examples that have step j
      - n_corr[j], n_inc[j] = counts of such responses
    """
    max_len = 0
