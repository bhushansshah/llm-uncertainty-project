...
Default output: one figure with three stacked panels (mean, variance, support counts) and
``sharex=True``. Use ``--separate_plots`` for standalone PNGs.
...
Usage:
  python scripts/val_step_agg_entropy_plot.py \\
    ...
    --val_size 60 \\
    --chunk_size 50 \\
    --output_path val_agg_entropy_vs_step.png
"""
...
def plot_val_mean_agg_entropy_vs_step(
...
def plot_val_variance_agg_entropy_vs_step(
    ...
    if np.any(mask_c):
        ax.plot(
            ...
            color="tab:green",
            linewidth=2,
            label="Val — Var(cum. entropy) (correct)",
        )
    if np.any(mask_i):
        ax.plot(
            j[mask_i],
            ...
            color="tab:orange",
            linewidth=2,
            label="Val — Var(cum. entropy) (incorrect)",
        )

    ax.set_xlabel("Step index (non-overlapping chunk)")
    ax.set_ylabel("Variance of cumulative chunk entropy")
    title = (
        f"Validation: variance of cumulative entropy vs step | chunk_size={chunk_size}\n"
...
def main() -> None:
    ...
    p.add_argument(
        "--output_path",
        type=str,
        default="val_agg_entropy_vs_step.png",
    ...
    if args.separate_plots:
        ...
        plot_val_mean_agg_entropy_vs_step(
            mean_corr,
            mean_inc,
            args.chunk_size,
            str(mean_only),
        ...
        plot_val_variance_agg_entropy_vs_step(
            var_corr,
            var_inc,
            args.chunk_size,
            str(var_path),
