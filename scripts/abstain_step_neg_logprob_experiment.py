...
Same grid as step-entropy abstention (`chunk_size`, `delta`, `noise`, `ground_threshold`),
but per-step uncertainty is the **mean −log p** of sampled tokens in each chunk (thinking only).
...
Requires per-token log probs under ``response.logprobs.token_logprobs`` (legacy) or
``response.logprobs.logprobs`` (newer dumps); see ``get_per_token_logprob_list`` in
``abstain_step_entropy.py``.
"""
...
def run_grid(
    ...
    for cs in chunk_sizes:
        csl, isl = [], []
        for d, seq in zip(val_data, thinking_nll):
    ...
    for cs in chunk_sizes:
        step_means_flat_by_cs[cs] = [
            chunk_step_means(seq, cs) for seq in thinking_nll
        ]
...
def plot_validation_step_neg_logprob_curves(
    ...
    title = (
        f"Validation: mean −log p vs step | chunk_size={chunk_size}, δ={delta}, noise={noise}"
    )
    if model_name:
        title = f"{model_name}\n{title}"
    ax.set_title(title)
...
def run_dataset_batch(
    ...
    """
    Batch mode: ``dataset_dir`` is ``<outputs_root>/<dataset_name>/`` (one folder per model).
    Writes ``avg_entropy.csv`` (summary only; same as pre-refactor batch) under
    ``abstaining_results_dir/<dataset>/<neg_logprob>/`` and plots under
    ``abstaining_plots_dir/<dataset>/<neg_logprob>/``.
    """
    ...
    for model_name, res_dir in model_dirs:
        ...
        n = len(data)
        if n <= val_size:
            print(f"SKIP: need more than val_size={val_size} examples, got {n}")
            continue
        n_usable = len(data)
        ...
        best = run_grid(val_data, save_csv=None, min_support_per_class=min_support_per_class)
        if not best:
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=int,
        default=3,
        help="Require at least this many correct AND incorrect val responses at a step for active/τ",
    )
    p.add_argument("--output_csv", type=str, default=None, help="Write full grid search CSV (single mode)")
    p.add_argument(
        "--plot_path",
        type=str,
        default=None,
