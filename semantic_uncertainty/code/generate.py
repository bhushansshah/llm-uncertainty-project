...
wandb.init(project=args.project, id=args.run_id, config=args, resume='allow')
    ...
    with torch.no_grad():
        ...
        for batch in tqdm.tqdm(dataloader):
            if args.dataset == 'trivia_qa':
                input_ids_raw = batch['input_ids']
                if torch.is_tensor(input_ids_raw):
                    input_ids = input_ids_raw.to(device)
                    if input_ids.dim() == 1:
                        input_ids = input_ids.unsqueeze(0)
                else:
                    input_ids = torch.cat(input_ids_raw).to(device).reshape(1, -1)
            else:
